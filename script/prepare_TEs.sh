...
display_usage() { 
        ...
        echo -e "Usage: bash prepare_TEs.sh -i <TE_coordiantes.bed> -g <Genome.fa> -d <directory of existing TE indexed provided by SPLITREADER>\n" 
        echo 
		echo -e "Example command: bash prepare_TEs.sh -i Test_data/TE_list/test_TE_coordinates.bed -g Genome.fa -d Test_data/TE_indexes"
		echo
} 
...
bowtie2-build TMP_FA/$ID.fa $INDEXES/$ID > /dev/null 2>&1
